...
CREATE INDEX IF NOT EXISTS works_id_idx 
    ON works (id);

CREATE INDEX IF NOT EXISTS work_references_work_id_idx 
    ON work_references (work_id);

CREATE INDEX IF NOT EXISTS rolap.works_issn_subject_doi_idx 
    ON works_issn_subject (doi);

CREATE INDEX IF NOT EXISTS rolap.issn_subject_h5_issn_idx 
    ON issn_subject_h5 (issn);

-- Step 5: Count the h-index of the cited journals for the authors in random_matching_authors
CREATE TABLE rolap.random_author_issn_hindex AS
